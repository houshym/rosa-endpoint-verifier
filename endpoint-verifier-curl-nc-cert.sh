...
#     REGION="$1"
# fi
PROXY_ADDRESS="https://proxy.example.com:3128"
...
for endpoint_with_port in "${endpoints[@]}"; do
    ...
    if nc -vzw1 $domain $port 2>&1 | grep -q 'succeeded'; then
        echo -e "${GREEN}Successfully connected to $url using nc${NC}"
    else
    ...
    # Use curl
    if [ "$port" == "443" ]; then
        if curl --connect-timeout 5 --silent --head --fail -x $PROXY_ADDRESS --cacert $CERTIFICATE_PATH "$url" > /dev/null 2>&1; then
            echo -e "${GREEN}Successfully connected to $url using curl${NC}"
        else
            echo -e "${RED}Failed to connect to $url using curl${NC}"
        fi
    else
        echo "Skipping curl for non-HTTPS endpoint"
    fi
done
